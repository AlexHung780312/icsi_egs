...
git clone https://github.com/cmusphinx/cmudict.git $dir/cmudict
...
cat $dir/cmudict/cmudict.symbols | sed s/[0-9]//g | \
...
grep -v ';;;' $dir/cmudict/cmudict.dict | \
 perl -ane 'if(!m:^;;;:){ s:(\S+)\(\d+\) :$1 :; s:  : :; print; }' | \
   sed s/[0-9]//g | tr '[:lower:]' '[:upper:]' | sort | uniq > $dir/lexicon1_raw_nosil.txt || exit 1;
...
gunzip -c $dir/wordlist.50k.gz | sort > $dir/wordlist.50k
...
#
cat local/icsi.dict >> $dir/lexicon2_raw_50k.txt
...
cat data/all/text | tr '[:lower:]' '[:upper:]' | \
